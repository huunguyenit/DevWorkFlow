...
/// <summary>
/// Thước pixel kiểu VS/WinForms Designer, dùng cho tab Design (DesignWebViewHost). <see cref="Offset"/>
/// mã hoá <c>(scroll − gốc bảng main FormTable) × zoom</c> (do host tính từ message 'designViewport') nên
/// tick 0 trùng mép bảng FormTable, không phải góc trang. OriginOffset giữ 0 (không dùng khái niệm "mép form"
/// của Designer WPF cũ đã gỡ).
/// </summary>
public sealed class PixelRuler : FrameworkElement
{
    ...
    /// <summary>Hệ số zoom hiện tại của WebView2 (1.0 = 100%).</summary>
    public static readonly DependencyProperty ZoomProperty =
        DependencyProperty.Register(
            nameof(Zoom), typeof(double), typeof(PixelRuler),
            new FrameworkPropertyMetadata(1.0, FrameworkPropertyMetadataOptions.AffectsRender));

    /// <summary>Vị trí cuộn hiện tại (DIP, đã nhân zoom) — 0 của thước theo dõi mép trang khi cuộn.</summary>
    ...
    protected override void OnRender(DrawingContext dc)
    {
        ...
        {
            // Phạm vi nhìn thấy theo px trang (0 = góc trang)
            var page_start = scroll / zoom;
            var page_end = (scroll + width) / zoom;
            var first = Math.Floor(page_start / minor) * minor;
            for (var page_px = first; page_px <= page_end + minor; page_px += minor)
            {
                var x = page_px * zoom - scroll;
                if (x < -1 || x > width + 1) continue;
                var is_major = IsMajorTick(page_px, major);
                var tick_h = is_major ? height : height * 0.45;
                dc.DrawLine(is_major ? MajorPen : MinorPen,
                    new Point(x, height - tick_h), new Point(x, height));
                if (is_major)
                {
                    var text = CreateText(FormatLabel(page_px), dpi);
        ...
        {
            var page_start = scroll / zoom;
            var page_end = (scroll + height) / zoom;
            var first = Math.Floor(page_start / minor) * minor;
            for (var page_px = first; page_px <= page_end + minor; page_px += minor)
            {
                var y = page_px * zoom - scroll;
    ...
    private static bool IsMajorTick(double page_px, double major)
    {
        var m = page_px % major;
