...
<!ENTITY UnitResult "@q">
<!ENTITY UnitFilter "@@0">
<!ENTITY UnitQuery "declare @q varchar(1024), @alias nvarchar(32); exec @@sysDatabaseName..FastBusiness$Fields$GetValue 'LF02', '&TransferID;', @alias output; select @alias = case when isnull(@alias, '') = '' then '&Tag;' else ltrim(@alias) end; exec FastBusiness$System$GetUnitFilter '', @@userID, @@admin, @alias, '&k;', 'ma_dvcs', 'ds_dvcs', '&UnitTag;dv', @q output&Parameters;; select '&amp;' + char(255) + ';' + &UnitResult; as value">
