...
<!DOCTYPE dir [
  ...
  <!ENTITY Controller "GNApprovalCategoryDetail">

  <!ENTITY k0 "@loai_duyet = $loai_duyet.OldValue and @ma_kieu = $ma_kieu.OldValue and @dvcs = $dvcs.OldValue and @ma_bp = $ma_bp.OldValue and @ma_hrbp = $ma_hrbp.OldValue and @tk = $tk.OldValue and @nh_kh1 = $nh_kh1.OldValue and @nh_kh2 = $nh_kh2.OldValue and @nh_kh3 = $nh_kh3.OldValue and @sl_tu = $sl_tu.OldValue and @ma_nt = $ma_nt.OldValue and @tien_tu = $tien_tu.OldValue and @loai_ct = $loai_ct.OldValue">
  <!ENTITY k1 "loai_duyet = @loai_duyet and ma_kieu = @ma_kieu and dvcs = @dvcs and ma_bp = @ma_bp and ma_hrbp = @ma_hrbp and tk = @tk and nh_kh1 = @nh_kh1 and nh_kh2 = @nh_kh2 and nh_kh3 = @nh_kh3 and sl_tu = @sl_tu and ma_nt = @ma_nt and tien_tu = @tien_tu and loai_ct = @loai_ct">
  <!ENTITY k2 "loai_duyet = $loai_duyet.OldValue and ma_kieu = $ma_kieu.OldValue and dvcs = $dvcs.OldValue and ma_bp = $ma_bp.OldValue and ma_hrbp = $ma_hrbp.OldValue and tk = $tk.OldValue and nh_kh1 = $nh_kh1.OldValue and nh_kh2 = $nh_kh2.OldValue and nh_kh3 = $nh_kh3.OldValue and sl_tu = $sl_tu.OldValue and ma_nt = $ma_nt.OldValue and tien_tu = $tien_tu.OldValue and loai_ct = $loai_ct.OldValue">
  <!ENTITY k3 "loai_duyet = @loai_duyet and ma_kieu = @ma_kieu and dvcs = @dvcs and ma_bp = @ma_bp and ma_hrbp = @ma_hrbp and tk = @tk and nh_kh1 = @nh_kh1 and nh_kh2 = @nh_kh2 and nh_kh3 = @nh_kh3 and sl_tu = @sl_tu and sl_den = @sl_den and ma_nt = @ma_nt and @tien_tu &lt;= tien_den and tien_tu &lt;= @tien_den and loai_ct = @loai_ct">
  <!ENTITY k4 "loai_duyet = @loai_duyet and ma_kieu = @ma_kieu and dvcs = @dvcs and ma_bp = @ma_bp and ma_hrbp = @ma_hrbp and tk = @tk and nh_kh1 = @nh_kh1 and nh_kh2 = @nh_kh2 and nh_kh3 = @nh_kh3 and @sl_tu &lt;= sl_den and sl_tu &lt;= @sl_den and ma_nt = @ma_nt and tien_tu = @tien_tu and tien_den = @tien_den and loai_ct = @loai_ct">
]>

<dir table="gnctkieuduyet" code="loai_duyet, ma_kieu, dvcs, ma_bp, ma_hrbp, nh_kh1, nh_kh2, nh_kh3, tk, ma_nt, sl_tu, tien_tu, loai_ct" order="loai_duyet, ma_kieu, dvcs, ma_bp, ma_hrbp, nh_kh1, nh_kh2, nh_kh3, tk, ma_nt, sl_tu, tien_tu, loai_ct" xmlns="urn:schemas-fast-com:data-dir">
  ...
  <fields>
    ...
    <field name="ma_kieu" isPrimaryKey="true" dataFormatString="@upperCaseFormat" allowNulls="false" inactivate="true" disabled="true">
      ...
      <items style="AutoComplete" controller="GNApprovalCategory" reference="ten_kieu%l" key="status = '1'" check="1 = 1" information="ma_kieu$gndmkieuduyet.ten_kieu%l"/>
    </field>
    ...
    <field name="ma_hrbp" isPrimaryKey="true" clientDefault="Default" allowNulls="false">
      <header v="Bộ phận" e="Department"></header>
      <items style="AutoComplete" controller="hreDepartment" reference="ten_hrbp%l" key="status = '1'" check="1 = 1" information="ma_bp$hrbp.ten_bp%l"/>
    </field>
    <field name="ten_hrbp%l" readOnly="true" external="true" clientDefault="Default" defaultValue="''">
      <header v="" e=""></header>
    </field>
    ...
    <field name="loai_ct" isPrimaryKey="true" allowNulls="false">
      ...
      <items style="AutoComplete" controller="VoucherType" reference="ten_loai_ct%l" key="ma_ct = (select ma_ct from gndmloaiduyet a left join gndmkieuduyet b on a.loai_duyet = b.loai_duyet where b.ma_kieu = '{$%c[ma_kieu]}') and status = '1'" check="ma_ct = (select ma_ct from gndmloaiduyet a left join gndmkieuduyet b on a.loai_duyet = b.loai_duyet where b.ma_kieu = '{$%c[ma_kieu]}')" information="loai_ct$dmloaict.ten_loai%l"/>
    </field>
  ...
  <views>
    <view id="Dir">
      ...
      <item value="11010: [ma_hrbp].Label, [ma_hrbp], [ten_hrbp%l]"/>
